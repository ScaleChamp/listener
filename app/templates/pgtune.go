...
var (
	postgresTuningMap = map[bool]map[int]map[uint64]string{
		false: {
			1: {
				1000000000: `max_connections = 512
...
max_wal_size = 4GB
`,
				2000000000: `max_connections = 512
...
max_parallel_workers = 2
`,
				8000000000: `max_connections = 512
shared_buffers = 2GB
...
`,
			...
			4: {
				16000000000: `max_connections = 512
shared_buffers = 4GB
...
`,
			...
			8: {
				32000000000: `max_connections = 512
shared_buffers = 8GB
...
`,
	...
	postgresConfWithoutTuning = map[string]string{
		"10": `# -----------------------------
...
ssl_key_file = '/etc/key.pem'
ssl_ca_file = '/etc/ca_bundle.pem'
...
archive_mode = on		# enables archiving; off, on, or always
				# (change requires restart)
archive_command = '/usr/local/bin/wal-push.sh %p'		# command to use to archive a logfile segment
...
var defaultPostgresConfigByVersion = map[string]string{
	"10": `# -----------------------------
# PostgreSQL configuration file
# -----------------------------
#
...
func PgConfByPlan(plan *models.Plan) string {
	byVirtualCPUs, ok := postgresTuningMap[plan.Rotational]
	if !ok {
		return defaultPostgresConfigByVersion[plan.Version]
	}
	byMemory, ok := byVirtualCPUs[plan.VirtualCPUs]
	if !ok {
		return defaultPostgresConfigByVersion[plan.Version]
	}
	tuning, ok := byMemory[plan.Memory]
	if !ok {
		return defaultPostgresConfigByVersion[plan.Version]
	}
	return postgresConfWithoutTuning[plan.Version] + tuning
}
