...
func (w *workflow) flowForRedis(task *models.Task) models.Steps {
	switch task.Action {
	case "create-redis-1":
		return models.Steps{
			...
			models.NewStep("setup-vm", w.setupRedis, []string{"instance_id", "node_id"}),
	...
	case "create-redis-2":
		return models.Steps{
			...
			// redibu backup-now
			models.NewStep("setup-first-vm", w.setupRedis, []string{"instance_id", "node1_id"}),
			...
			models.NewStep("setup-second-vm", w.setupRedisAsSlave, []string{"instance_id", "node2_id", "node1_id"}),
	...
	case "poweroff-redis-2": // and any other with 2 nodes
		return models.Steps{
			models.NewStep("destroy-monitor-1", w.disableMonitoringByNodeId, []string{"node1_id"}),
	...
	case "destroy-redis-2": // and any other with 2 nodes
		return models.Steps{
			models.NewStep("destroy-monitor-1", w.disableMonitoringByNodeId, []string{"node1_id"}),
			models.NewStep("destroy-monitor-2", w.disableMonitoringByNodeId, []string{"node2_id"}),
			models.NewStep("destroy-dns-1", w.destroyDNSRecordByNodeId, []string{"node1_id"}),
	...
	case "update-redis-1-to-1":
		...
		if task.Metadata["plan_id"] != nil {
			commands = append(commands,
				...
				models.NewStep("setup-ufw-for-second-vm", w.allowAccessFromMultipleIP, []string{"instance_id", "new_node_id"}), // updating whitelist for new instance
				models.NewStep("setup-new-vm", w.setupRedis, []string{"instance_id", "new_node_id"}),
	...
	case "update-redis-1-to-2":
		...
		if task.Metadata["eviction_policy"] != nil {
			commands = append(commands,
				models.NewStep("update-redis-master-eviction-policy", w.updateRedisCompatEviction, []string{"instance_id", "node1_id"}),
			)
		}
		if task.Metadata["plan_id"] != nil {
			commands = append(commands,
				...
				models.NewStep("setup-new-master", w.setupRedis, []string{"instance_id", "new_master_id"}),
				...
				models.NewStep("setup-second-vm", w.setupRedisAsSlave, []string{"instance_id", "new_slave_id", "new_master_id"}),
	...
	case "update-redis-2-to-1":
		...
		if task.Metadata["plan_id"] != nil {
			commands = append(commands,
				...
				models.NewStep("setup-new-master", w.setupRedis, []string{"instance_id", "new_node_id"}),
	...
	case "update-redis-2-to-2":
		...
		return append(commands, models.NewStep("finish", w.finishSetupForInstanceId, []string{"instance_id"}))
	case "recover-redis-master-false":
		return models.Steps{
			...
			models.NewStep("setup-vm", w.setupRedis, []string{"instance_id", "new_node_id"}),
	...
	case "recover-redis-master-false-slave-true":
		return models.Steps{
			...
			models.NewStep("setup-second-vm", w.setupRedisAsSlave, []string{"instance_id", "node2_id", "node0_id"}),
	...
	case "recover-redis-master-false-slave-false":
		return models.Steps{
			models.NewStep("destroy-monitor-1", w.disableMonitoringByNodeId, []string{"node0_id"}),
			...
			models.NewStep("setup-first-vm", w.setupRedis, []string{"instance_id", "new_node1_id"}),
			models.NewStep("setup-second-vm", w.setupRedisAsSlave, []string{"instance_id", "new_node2_id", "new_node1_id"}),
	...
	case "recover-redis-master-true-slave-false":
		return models.Steps{
			models.NewStep("destroy-monitor-1", w.disableMonitoringByNodeId, []string{"node1_id"}),
			models.NewStep("create-new-vm", w.createNodeByInstanceIdAndPlanId, []string{"instance_id", "plan_id"}, []string{"node2_id"}),
			models.NewStep("upload-pgp", w.uploadPublicPGP, []string{"instance_id", "node2_id"}),
			...
			models.NewStep("setup-new-vm", w.setupRedisAsSlave, []string{"instance_id", "node2_id", "node0_id"}),
			models.NewStep("update-dns-record", w.upgradeDNSRecordByNodeIdAndNewNodeId, []string{"instance_id", "node1_id", "node2_id"}),
			models.NewStep("destroy-node-1", w.destroyNodeById, []string{"node1_id"}),
		}
	default:
		return w.flowForKeyDB(task)
	}
}
