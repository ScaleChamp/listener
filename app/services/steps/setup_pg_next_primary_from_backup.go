...
type SetupPgNextPrimaryFromLatestBackup struct {
...
func (o *SetupPgNextPrimaryFromLatestBackup) Do(instanceId, primaryId, replicaId uuid.UUID) error {
	...
	if err := o.executor.Run(primary, `psql postgres://postgres:%s@localhost -c "SELECT pg_create_physical_replication_slot('slot2', true);"`, instance.Secret); err != nil {
		return err
	}
	if err := o.executor.Run(primary, `touch /tmp/maintenance`); err != nil {
		return err
	}
...
func (*SetupPgNextPrimaryFromLatestBackup) Undo(uuid.UUID, uuid.UUID, uuid.UUID) error {
	return nil
}

func NewSetupPgNextPrimaryFromLatestBackup(
	executor *executor.Executor,
	nodeRepository components.NodeRepository,
	instanceRepository components.InstanceRepository,
	encryptionKeyRepository components.EncryptionKeyRepository,
	...
	return &SetupPgNextPrimaryFromLatestBackup{
		executor,
		nodeRepository,
		instanceRepository,
